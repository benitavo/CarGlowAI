    ...
    <div className="pb-24 lg:pb-12">
      ...
      <div className="px-6 lg:px-10 py-6 max-w-[1480px]">
          ...
          {CATS.map(c => (
            <button key={c} onClick={() => setCat(c)}
              className={cn('px-3.5 py-1.5 rounded-full text-sm border transition-colors',
                cat === c ? 'border-sage-500/40 bg-sage-500/15 text-sage-300' : 'border-white/[0.08] text-offwhite/55 hover:border-white/[0.16] hover:text-offwhite'
              )}>{c}</button>
          ))}
        </div>
          ...
          {filtered.map(s => (
            <button key={s.id} onClick={() => setPicked(s.id)}
              className={cn('group relative aspect-[4/5] rounded-2xl overflow-hidden ring-2 transition-all',
                picked === s.id ? 'ring-sage-500' : 'ring-transparent hover:ring-white/20'
              )}>
              ...
              {picked === s.id && (
                <div className="absolute top-2 right-2 w-6 h-6 rounded-full bg-sage-500 flex items-center justify-center shadow-sage-sm">
                  <Check className="w-3.5 h-3.5 text-midnight" strokeWidth={3} />
                </div>
              )}
              <div className="absolute bottom-0 left-0 right-0 p-3 text-left">
                <div className="text-[10px] font-semibold tracking-widest uppercase text-sage-400/80 mb-0.5">{s.category}</div>
        ...
        <div className="mt-8 flex items-center gap-3">
          <button className="h-11 px-6 rounded-xl bg-sage-500 hover:bg-sage-600 text-midnight font-semibold text-sm shadow-sage-sm transition-all">
            Save as default
          </button>
