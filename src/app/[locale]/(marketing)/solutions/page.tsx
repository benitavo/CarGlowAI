      ...
      <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
        {SOLUTIONS.map((sol) => (
          <Link key={sol.title} href={sol.href}
            className="group card-noise rounded-3xl p-8 hover:border-sage-500/30 transition-all duration-300">
            <div className="w-12 h-12 rounded-2xl bg-white/[0.05] border border-white/[0.08] flex items-center justify-center mb-5 group-hover:border-sage-500/30 transition-all">
              <sol.icon className="w-5 h-5 text-offwhite/60 group-hover:text-sage-400 transition-colors" />
            </div>
            <h2 className="text-xl font-display font-semibold text-offwhite mb-2 group-hover:text-sage-300 transition-colors">{sol.title}</h2>
            ...
            <span className="text-sm font-medium text-offwhite/40 group-hover:text-sage-400 flex items-center gap-1 transition-colors">
