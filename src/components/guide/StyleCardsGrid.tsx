  ...
  return (
    <div className="not-prose grid sm:grid-cols-2 gap-3 my-6">
      {GARDEN_STYLES.map((style) => (
        <div key={style.slug} className="rounded-2xl border border-midnight/[0.08] bg-cream-50 p-4">
          <div className="flex items-center gap-2 mb-1.5">
            <span className="text-lg">{style.emoji}</span>
            <h3 className="font-display font-semibold text-midnight text-[15px]">{style.name}</h3>
          </div>
